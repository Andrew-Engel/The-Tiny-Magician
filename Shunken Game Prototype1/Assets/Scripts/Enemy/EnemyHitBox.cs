    ...
    public void DamageAbsorbed(string spell)
    {
        switch (spell)
        {
            ...
            case "IceBlast":
                ...
                int iceBlastDamage;
                iceBlastDamage = UnityEngine.Random.Range(MagicCasting.iceBlastDamage - 3, MagicCasting.iceBlastDamage * +3);
                bool isIceBlastCritical = UnityEngine.Random.Range(0, 100) < 10;
                if (isIceBlastCritical) iceBlastDamage *= 2;

                if (enemyBehavior != null)
                    enemyBehavior.enemyLives -= iceBlastDamage;

                DamagePopup.Create(this.transform.position, iceBlastDamage, isIceBlastCritical);
                break;
        }
    }
