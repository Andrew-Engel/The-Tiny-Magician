...
public class MagicCasting : MonoBehaviour
{
    ...
    public static int iceBlastDamage = 20;
    ...
    Dictionary<string, int> spellManaCost = new Dictionary<string, int>
    {
        ...
        {Spells.AirDash.ToString(),15},
        {Spells.IceBlast.ToString(),20 }
    };
   ...
   new  AudioSource audio;
    ...
    void Start()
    {
        ...
        icePathSpells = GetComponent<IcePathSpells>();
    ...
    {
        if (manaBarSystem == null) manaBarSystem = GameObject.Find("GameManager").GetComponent<ManaBarSystem>();
        if (Time.timeScale > 0)
        {
            ...
            if ((enemyLockOn.lockedOnEnemy) || (move.y >= 0 && !enemyLockOn.lockedOnEnemy))
            {
                ...
                switch (spellUsed)
                {
                    ...
                    case "AirEscape":
                        ...
                        break;
                    case "AirDash":
                        if (manaBarSystem.mana >= spellManaCost["AirDash"])
                        {
                            casting = true;
                            TriggerOnSpellUseEvent();
                            if (Sneaking.playerSneaking) sneakingSystem.Crouch();
                            airPathSpells.AirDashForwards();
                        }
                        break;
                    case "IceBlast":
                        if (manaBarSystem.mana >= spellManaCost["IceBlast"])
                        {
                            Debug.Log($"IceBlast cost: " + spellManaCost["IceBlast"]);
                            Debug.Log($"CurrentMana: " + manaBarSystem.mana);
                            casting = true;
                            TriggerOnSpellUseEvent();
                            if (Sneaking.playerSneaking) sneakingSystem.Crouch();
                            IceBlast();
    ...
    private IEnumerator StandStill(float animationDuration)
    {
    ...
    private void EarthWave()
    {
        ...
        if (Sneaking.playerSneaking)
            sneakingSystem.Crouch();
        TriggerOnSpellUseEvent();
        StartCoroutine(StandStill(magicAnimationDuration));
    ...
    private void IceBlast()
    {
        ...
        if (Sneaking.playerSneaking)
            sneakingSystem.Crouch();
       StartCoroutine(icePathSpells.IceBlast());
        StartCoroutine(StandStill(icePathSpells.blastDuration));
