   ...
   new  AudioSource audio;
    ...
    private void SwitchSpells()
    {
        if (!spellInProgress)
        {
            ...
            spellIndex++;
            if (spellIndex > (skillTree.spellsUnlocked.Count - 1))
            {
                spellIndex = 0;
                spellUsed = skillTree.spellsUnlocked[spellIndex];
                if (OnSpellChange != null)
                    OnSpellChange(this, new OnSpellChangeEventArgs { currentSpell = spellUsed });
            }
            else
            {
                spellUsed = skillTree.spellsUnlocked[spellIndex];
